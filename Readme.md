...
```
Rebol中提供了多达50种数据类型，RML目前实现了常用的一些基本数据类型，从左到右分别是 `单字`，`设字`，`整数`，`小数`，`逻辑`，`字符串`，`字符`，`方块`，`圆块`，`对象`，`路径`，`文件`，`二元` 类型，数据类型有对应的字面表达格式，主要通过空白字符分隔。集合类型首先被视为单个Token，内部另行分割。
### 赋值
```
i: 123	j: 456
x: j: 789
...
gitee: [https://gitee.com/NjinN/RML](https://gitee.com/NjinN/RML<br/>)<br/>
